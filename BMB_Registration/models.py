...
PRESENTATION = (("poster", "poster"), ("talk", "talk"), ("decline", "decline"))
...
ATTENDANCE = (("both", "both"), ("thursday", "thursday"), ("friday", "friday"))
...
class User(models.Model):
    ...
    lab = models.ForeignKey(PI, on_delete=models.CASCADE)
    shirt_size = models.CharField(choices=TSHIRT_SIZES, max_length=4)
    presentation = models.CharField(
        choices=PRESENTATION,
        default="decline",
        max_length=7,
        help_text="Years 3 and above must present a poster.",
    )
...
class Submission(models.Model):
    ...
    PI = models.ForeignKey(PI, blank=True, null=True, on_delete=models.CASCADE)
...
class PosterRank(models.Model):
    ...
    poster_number = models.ForeignKey(
        Submission, to_field="poster_number", on_delete=models.CASCADE
    )
...
def user_directory_path(instance, filename):
    ...
    return "uploads/{0}_{1}/{2}".format(
        instance.user.last_name, instance.user.first_name, filename
    )
